...
async def start_lavalink(event: hikari.ShardReadyEvent) -> None:
    ...
    youtube = build('youtube', 'v3', static_discovery=False,
...
async def _join(ctx: lightbulb.Context) -> Optional[hikari.Snowflake]:
    ...
    voice_state = [state[1] for state in filter(
        lambda i: i[0] == ctx.author.id, states.items())]
    bot_voice_state = [state[1] for state in filter(
        lambda i: i[0] == ctx.bot.get_me().id, states.items())]
...
async def chill(ctx: lightbulb.Context) -> None:
    ...
    while True:
        ...
        if rand_vid < 50:
            # id
            vid_id = res['items'][rand_vid]['snippet']['resourceId']['videoId']
...
async def voice_state_update(event: hikari.VoiceStateUpdateEvent) -> None:
    ...
    states = plugin.bot.cache.get_voice_states_view_for_guild(
        cur_state.guild_id).items()
    bot_voice_state = [state[1] for state in filter(
        lambda i: i[0] == plugin.bot.get_me().id, states)]
    ...
    cnt_user = len([state[0] for state in filter(
        lambda i: i[1].channel_id == bot_voice_state[0].channel_id, states)])
